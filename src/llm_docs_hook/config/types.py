...
class LLMConfig(BaseModel):
    ...
    temperature: float = Field(
        default=0.1, ge=0.0, le=2.0, description="Generation temperature (0.0 to 2.0)"
    )
    ...
    def validate_model_format(cls, model_value: str) -> str:
        ...
        if "/" not in model_value:
            raise ValueError(
                "Model must be in format 'provider/model' (e.g., 'openai/gpt-4o-mini')"
            )
        return model_value
...
class DocstringConfig(BaseModel):
    ...
    style: Literal["google", "numpy"] = Field(
        default="google", description="Docstring style format"
    )
    include_types: bool = Field(
        default=True, description="Include type annotations in docstrings"
    )
    include_examples: bool = Field(
        default=False, description="Include usage examples in docstrings"
    )
...
class Config(BaseModel):
    ...
    docstring: DocstringConfig = Field(
        default_factory=DocstringConfig, description="Docstring generation preferences"
    )
    files: FileConfig = Field(
        default_factory=FileConfig, description="File processing patterns"
    )
