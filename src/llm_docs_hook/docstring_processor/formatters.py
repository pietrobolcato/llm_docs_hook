        ...
        if ':' in line:
            ...
            if len(parts) == 2:
                ...
                if ('(' in param_part and ')' in param_part) or \
                   (param_part.replace('_', '').replace('-', '').isalnum() and 
                    len(param_part.split()) <= 3):  # Simple param name pattern
                    return True
...
class NumpyFormatter(DocstringFormatter):
    ...
    def _is_numpy_section_header(self, line: str, lines: List[str], index: int) -> bool:
        ...
        """
        numpy_sections = [
            'Parameters', 'Returns', 'Yields', 'Raises', 'See Also', 
            'Notes', 'Examples', 'Attributes', 'Methods'
        ]
        ...
        if (index + 1 < len(lines) and 
            ...
            all(c == '-' for c in lines[index + 1].strip())):
            return True
